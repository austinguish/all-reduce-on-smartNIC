...
#define MAX_BURSTS_X_QUEUE 1024
...
allocate_mempool_gpu(struct gpu_pipeline *pipe, const uint32_t total_nb_mbufs)
{
	...
	if (pipe->is_host_mem) {
		...
		if (ret < 0)
			APP_EXIT("Unable to gpudev register addr 0x%p, ret %d", pipe->ext_mem.buf_ptr, ret);
	} else {
		...
		ret = rte_extmem_register(pipe->ext_mem.buf_ptr, pipe->ext_mem.buf_len, NULL, pipe->ext_mem.buf_iova,
					  GPU_PAGE_SIZE);
		if (ret)
			APP_EXIT("Unable to register addr 0x%p, ret %d", pipe->ext_mem.buf_ptr, ret);
	}
...
gpu_fini(struct gpu_pipeline *pipe)
{
	...
	if (pipe->is_host_mem) {
		...
		if (ret < 0)
			APP_EXIT("Unable to unregister addr 0x%p, ret %d", ext_mem->buf_ptr, ret);
	} else {
		ret = rte_gpu_mem_free(pipe->gpu_id, ext_mem->buf_ptr);
		if (ret < 0)
			APP_EXIT("Failed to free GPU memory addr 0x%p, ret %d ", ext_mem->buf_ptr, ret);
	}
}
